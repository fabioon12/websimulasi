    ...
    <aside 
        ...
        <div class="p-6">
            ...
            <nav class="space-y-1">
                ...
                <a href="{{ route('guru.proposal.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl font-bold transition {{ Request::is('guru/proposal*') ? 'sidebar-active' : 'text-slate-500 hover:bg-slate-50' }}">
                    <i class="fas fa-file-signature w-5 shrink-0 text-center"></i>
                    <span x-show="!sidebarCollapsed" x-transition.opacity>Persetujuan Proyek</span>
                    {{-- Badge Notifikasi jika ada proposal pending --}}
                    @php
                        $pendingCount = \App\Models\Proposal::where('guru_id', Auth::id())->where('status', 'pending')->count();
                    @endphp
                    @if($pendingCount > 0)
                        <span x-show="!sidebarCollapsed" class="ml-auto bg-rose-500 text-white text-[10px] px-2 py-0.5 rounded-full animate-pulse">
                            {{ $pendingCount }}
                        </span>
                    @endif
                </a>
