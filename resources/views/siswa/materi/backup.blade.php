            ...
            <div class="relative">
                ...
                <div x-show="activeTab === 'materi'" ...>
                    <article class="max-w-4xl mx-auto">
                        <h2 class="text-4xl font-black text-slate-900 mb-10 ...">{{ $subMateriAktif->judul }}</h2>
                        
                        {{-- Pastikan class prose ada di sini --}}
                ...
                <div x-show="activeTab === 'coding'" x-cloak x-transition:enter="transition ease-out duration-300" class="grid grid-cols-1 xl:grid-cols-12 gap-8 min-h-[700px]">
                    {{-- Tambahkan overflow-hidden agar elemen anak tidak keluar batas --}}
                    <aside class="xl:col-span-4 bg-white rounded-[3rem] border border-slate-100 p-8 shadow-sm overflow-hidden flex flex-col">
                        <h4 class="text-[11px] font-black text-slate-900 uppercase tracking-widest mb-4">Instruksi Tugas</h4>
                        
                        {{-- Tambahkan overflow-y-auto (scroll jika teks terlalu panjang ke bawah) --}}
                        {{-- Tambahkan break-words dan prose-tight agar konten rapi --}}
                        <div class="text-xs font-bold text-slate-500 leading-relaxed prose prose-sm max-w-none break-words overflow-x-hidden overflow-y-auto pr-2">
                            {!! $subMateriAktif->instruksi_coding ?? 'Gunakan editor di samping untuk mencoba kode.' !!}
                        </div>
                    </aside>

                    <div class="xl:col-span-8">
                        {{-- ... (Bagian Editor tetap sama) ... --}}
...
<style>
    ...
    /* Garis bawah tipis untuk link di dalam materi */
    .prose a {
